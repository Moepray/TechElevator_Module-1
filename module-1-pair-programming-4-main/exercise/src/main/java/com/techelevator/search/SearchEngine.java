...
public class SearchEngine {
	...
	public void indexFiles() throws SearchEngineException, Exception {
		...
		//I changed this to a for each loop
		for(String files : fileNames){
			//this is appending a folder/file
			File file = new File(sd.getFolder() + "/" + files);

			try(Scanner scanner = new Scanner(file)){
				while(scanner.hasNextLine()){
					indexWords(fileId, scanner.nextLine());
				}
				fileId++;
			}catch (FileNotFoundException e) {
				throw new SearchEngineException("Exception: File '" + file + "' is not found.");
			}
		}
